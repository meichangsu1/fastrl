...
class FSDPSGLangShardingManager(BaseShardingManager):
    ...
    async def update_drafter_weights(self, drafter_params):
        if drafter_params is None or len(drafter_params) == 0:
            return

        logger.debug(f"Updating Eagle drafter weights with {len(drafter_params)} parameters")
        ...
        if self.device_mesh["infer_tp"].get_local_rank() == 0:
            await self.inference_engine.flush_cache()
            logger.debug("Drafter weights updated successfully")
    ...
    def compute_base_hidden_states(self, input_ids: torch.Tensor, attention_mask: torch.Tensor) -> torch.Tensor:
        """Compute base model last hidden states for given sequences.

        Runs the wrapped base module in eval no-grad mode and returns
        the last hidden states (batch, seq_len, hidden_size).
        """
    ...
    async def wake_up(self):
        ...
        # # The drafter might still be on GPU from previous training or initialization
        # if self.offload_param and self.drafter_module is not None:
        #     try:
        #         offload_fsdp_model_to_cpu(self.drafter_module)
        #         get_torch_device().empty_cache()
        #         logger.debug("Pre-emptively offloaded drafter before loading actor weights")
        #     except Exception as e:
        #         logger.debug(f"Failed to pre-offload drafter (may already be offloaded): {e}")
        ...
        # Update Eagle drafter weights if available
        if self.drafter_module is not None:
            logger.debug("Extracting and updating Eagle drafter weights")
            # if self.offload_param:
            ...
            await self.update_drafter_weights(drafter_params)
            log_gpu_memory_usage("After sync drafter weights in sharding manager", logger=logger)
    ...
    def preprocess_data(self, data: DataProto) -> DataProto:
        ...
        # NOTE: Current implementation doesn't consider FSDP with torch micro-dp
