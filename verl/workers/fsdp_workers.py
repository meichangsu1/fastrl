...
from verl.utils.model import compute_position_id_with_mask, get_generation_config, print_model_size, update_model_config
...
class ActorRolloutRefWorker(Worker, DistProfilerExtension):
    ...
    def _build_drafter_model(self):
        ...
        if spec_model_path and os.path.exists(spec_model_path):
            ...
            for pattern in allow_patterns:
                hf_weights_files += glob.glob(os.path.join(spec_model_path, pattern))
        ...
        fsdp_kwargs = {
            "mesh": self.drafter_device_mesh,
            "mp_policy": mp_policy,
            "offload_policy": None,
        }
        log_gpu_memory_usage("Inside building drafter model (Before FSDP2)", logger=logger)
        ...
        fsdp2_load_full_state_dict(drafter_module, full_state, self.drafter_device_mesh, None)
        drafter_module_fsdp = drafter_module

        logger.debug("Draft model built successfully")

        # Build drafter optimizer
        rollout_config = self.config.rollout
        drafter_train_config = rollout_config.speculative.train.copy()
        ...
        drafter_optimizer = optim.AdamW(
            [p for p in drafter_module_fsdp.parameters() if p.requires_grad],
            lr=drafter_train_config.optim.lr,
            betas=(0.9, 0.95),
            weight_decay=drafter_train_config.optim.get("weight_decay", 1e-2),
        )

        total_steps = drafter_train_config.optim.get("total_training_steps", 0)
        num_warmup_steps = int(drafter_train_config.optim.get("lr_warmup_steps", 1000))
        warmup_style = drafter_train_config.optim.get("warmup_style", "constant")
        min_lr_ratio = drafter_train_config.optim.get("min_lr_ratio", 0.0)
        num_cycles = drafter_train_config.optim.get("num_cycles", 0.5)
        if num_warmup_steps < 0:
            num_warmup_steps_ratio = drafter_train_config.optim.get("lr_warmup_steps_ratio", 0.0)
            num_warmup_steps = int(num_warmup_steps_ratio * total_steps)

        if self.rank == 0:
            print(f"Draft model - Total steps: {total_steps}, num_warmup_steps: {num_warmup_steps}")

        if warmup_style == "constant":
            drafter_lr_scheduler = get_constant_schedule_with_warmup(
                optimizer=drafter_optimizer, num_warmup_steps=num_warmup_steps
            )
        elif warmup_style == "cosine":
            drafter_lr_scheduler = get_cosine_schedule_with_warmup(
                optimizer=drafter_optimizer,
                num_warmup_steps=num_warmup_steps,
                num_training_steps=total_steps,
                min_lr_ratio=min_lr_ratio,
                num_cycles=num_cycles,
            ...
            drafter_lr_scheduler = get_linear_schedule_with_warmup(
                optimizer=drafter_optimizer,
                num_warmup_steps=num_warmup_steps,
                num_training_steps=total_steps,
            )
    ...
    def _build_rollout(self, trust_remote_code=False):
        ...
        # Initialize eagle model if speculative decoding is enabled
        enable_drafter_training = (
            hasattr(self.config.rollout, "speculative")
            and self.config.rollout.speculative.get("enable", False)
            and hasattr(self.config.rollout.speculative, "train")
            and self.config.rollout.speculative.train.get("enable_drafter_training", False)
        )
        if enable_drafter_training:
            ...
            if self._is_offload_param:
                offload_fsdp_model_to_cpu(drafter_module_fsdp)
                log_gpu_memory_usage("After offload actor model during init", logger=logger)

            if self._is_offload_optimizer:
                offload_fsdp_optimizer(optimizer=drafter_optimizer)
                log_gpu_memory_usage("After offload actor optimizer during init", logger=logger)
        else:
            logger.info("Drafter training is disabled. Skip background trainer initialization")

        log_gpu_memory_usage("Before rollout wake_up during rollout init", logger=logger)

        if rollout_name == "hf":
            self._register_dispatch_collect_info("rollout", dp_rank=self.rank, is_collect=True)
        else:
            is_collect = rollout_device_mesh["infer_tp"].get_local_rank() == 0
            self._register_dispatch_collect_info("rollout", dp_rank=self.rollout_dp_rank, is_collect=is_collect)
    ...
    def compute_log_prob(self, data: DataProto):
        ...
        with self.ulysses_sharding_manager:
            with adapter_ctx:
                result = self.actor.compute_log_prob(data=data, calculate_entropy=True, return_hidden_states=return_hidden_states)
